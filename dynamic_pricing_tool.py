...
def calculate_npl_premium_bps(npl_pct, threshold, multiplier, cap_bps):
    if npl_pct <= threshold:
        return 0
    raw = (npl_pct - threshold) * multiplier * 100
...
def calculate_risk_bps(params, weights):
    ...
    if md < 90:
        ...
        v = weights["maturity_120"]; label = f"120 gun"
    else:
    ...
    with st.expander("Yeni Kaynak Ekle", expanded=False):
        ...
        if st.button("Ekle", type="primary"):
            st.session_state.funding_sources.append({
                ...
                "fx_rate": new_fx if new_currency == "TL" else 1.0, "annual_rate": new_rate,
            })
            st.rerun()
    ...
    for i, src in enumerate(st.session_state.funding_sources):
        ...
        if not delete:
            updated_sources.append({
                ...
                "fx_rate": fx_rate if currency == "TL" else 1.0, "annual_rate": rate,
            })
...
# ══════════════════════════════════════════════
with tabs[1]:
    st.header("Strateji Ekibi - Yillik Hedefler & Makro Mod")
    ...
    with c1:
        ...
        s["avg_maturity_days"] = st.number_input("Hedef Ortalama Vade (Gun)", value=s["avg_maturity_days"], step=10,
                                                   help="Portfoy agirlikli ortalama vade hedefi")
    ...
    with c2:
        ...
        s["target_portfolio_gross_pct"] = st.number_input(
            ...
            help="Tum satislarin agirlikli ortalama brut kar hedefi. Ornek: 25% yillik."
        )
        s["npl_threshold_pct"] = st.number_input(
            "NPL Fiyatlamaya Yansima Esigi (%)",
            value=s["npl_threshold_pct"], step=0.5,
            help="Bu esigi gecen NPL orani fiyata prim olarak yansir."
        )
        s["npl_pct_new"] = st.number_input("Beklenen NPL % - Yeni Musteri", value=s["npl_pct_new"], step=0.1)
        s["npl_pct_repeat"] = st.number_input("Beklenen NPL % - Tekrar Musteri", value=s["npl_pct_repeat"], step=0.1)

    st.session_state.strategy = s

    # Strateji Hesaplama Ozeti
    st.divider()
    ...
    target_gross_monthly = round(target_monthly_rate - base_cost_monthly2, 4)
    ...
    col1.metric(
        "Hedef Portfoy Brut Kar (Yillik)",
        f"%{s['target_portfolio_gross_pct']:.1f}",
        help="Board hedefi"
    )
    col2.metric(
        "Hedef Ortalama Vade",
        f"{s['avg_maturity_days']} gun ({avg_maturity_months:.1f} ay)",
    )
    col3.metric(
        "Hedef Aylık Oran",
        f"%{target_monthly_rate:.2f} / ay",
        help=f"= Brut Kar Hedefi / Ort. Vade = {s['target_portfolio_gross_pct']}% / {avg_maturity_months:.1f} ay"
    )
    col4.metric(
        "Makro Duzeltmeli Aylik Oran",
        f"%{adjusted_monthly_rate:.2f} / ay",
        delta=f"Makro: {macro_bps2:+d} bps"
    )

    st.divider()
    st.subheader("Vade-Brut Kar Simulasyonu")
    st.caption(f"Hedef aylik oran **%{target_monthly_rate:.2f}** uzerinden farkli vadelerde beklenen brut kar:")
    ...
    for days in [30, 60, 90, 120, 150, 180]:
        ...
        sim_data.append({
            ...
            "Hedef mi?": "✅" if days == s["avg_maturity_days"] else ""
        })
    st.dataframe(pd.DataFrame(sim_data), use_container_width=True, hide_index=True)

    npl_new_bps = calculate_npl_premium_bps(s["npl_pct_new"], s["npl_threshold_pct"], w["npl_multiplier"], w["npl_cap_bps"])
    npl_rep_bps = calculate_npl_premium_bps(s["npl_pct_repeat"], s["npl_threshold_pct"], w["npl_multiplier"], w["npl_cap_bps"])
    if npl_new_bps > 0 or npl_rep_bps > 0:
        st.warning(f"NPL Primi Aktif — Yeni Musteri: +{npl_new_bps} bps | Tekrar Musteri: +{npl_rep_bps} bps")
    else:
        st.success(f"NPL esik altinda — Fiyatlamaya NPL primi yansimaz (Esik: %{s['npl_threshold_pct']})")
...
with tabs[2]:
    ...
    with col_input:
        ...
        # NPL
        npl_pct = s3["npl_pct_new"] if customer_type == "Yeni Musteri" else s3["npl_pct_repeat"]
        npl_bps = calculate_npl_premium_bps(npl_pct, s3["npl_threshold_pct"], w3["npl_multiplier"], w3["npl_cap_bps"])

        st.markdown("---")
        st.caption(f"NPL ({customer_type}): **%{npl_pct}** → Prim: **+{npl_bps} bps**")
    ...
    npl_pct_val = bps_to_pct(npl_bps)
    risk_pct3 = bps_to_pct(risk_bps)

    # Aylık oran hesabı
    maturity_months = maturity_days / 30
    # Hedef aylık oran + risk/npl/makro ayarlamaları aylığa bölünerek eklenir
    risk_monthly_adj = round(risk_pct3 / maturity_months, 4) if maturity_months > 0 else 0
    npl_monthly_adj = round(npl_pct_val / maturity_months, 4) if maturity_months > 0 else 0
    macro_monthly_adj = round(macro_pct3 / maturity_months, 4) if maturity_months > 0 else 0

    final_monthly_rate = round(base_cost_monthly3 + target_monthly_rate3 + risk_monthly_adj + npl_monthly_adj + macro_monthly_adj, 4)
    final_annual_rate = round(final_monthly_rate * 12, 4)
    vade_gross_pct = round(final_monthly_rate * maturity_months, 4)
    min_monthly_rate = round(base_cost_monthly3 + target_monthly_rate3, 4)

    with col_result:
        st.subheader("Fiyatlama Selalesi (Waterfall)")

        st.markdown("**Aylik Oran Hesabi:**")
        waterfall = [
            ...
            ("NPL Primi Aylik Etkisi", npl_monthly_adj, "npl"),
        ...
        for label, val, kind in waterfall:
            ...
            sign = "+" if val >= 0 else ""
            if kind == "maliyet":
                st.markdown(f"🔵 **{label}:** `{sign}{val:.3f}%`")
            elif kind == "hedef":
                st.markdown(f"🟢 **{label}:** `{sign}{val:.3f}%`")
            elif kind == "risk":
                color = "🔴" if val > 0 else "🟢"
                st.markdown(f"{color} **{label}:** `{sign}{val:.3f}%` ({risk_bps:+d} bps / {maturity_months:.1f} ay)")
            elif kind == "npl":
                color = "🔴" if val > 0 else "⚪"
                st.markdown(f"{color} **{label}:** `{sign}{val:.3f}%` ({npl_bps:+d} bps)")
            elif kind == "makro":
                color = "🟡"
                st.markdown(f"{color} **{label}:** `{sign}{val:.3f}%` ({macro_bps3:+d} bps)")
        ...
        if diff_from_target >= 0:
            st.success(f"Bu islem portfoy hedefini **{diff_from_target:+.2f}%** asıyor.")
        else:
        ...
        override_monthly = st.number_input(
            ...
            help="Yetkili kisi aylık oranı manuel revize edebilir."
        )
        ...
        if abs(override_monthly - final_monthly_rate) > 0.001:
            st.warning(f"Override: Sistem onerisinden {override_monthly - final_monthly_rate:+.3f}%/ay sapma | Vade brut kar: %{override_gross:.2f}")
...
with tabs[3]:
    ...
    approved = [d for d in deals if d["Durum"] == "Onaylandi"]
    ...
    with g1:
        ...
        progress_val = min(sales_progress / 100, 1.0)
        st.progress(progress_val)
        if sales_progress >= 100:
            st.success("Satis hedefi asildi!")
        elif sales_progress >= 75:
            st.warning(f"Hedefe %{100-sales_progress:.1f} kaldi")
        else:
            st.error(f"Hedefe %{100-sales_progress:.1f} kaldi")
    ...
    with g2:
        ...
        if approved:
            maturity_progress = min(weighted_maturity / s4["avg_maturity_days"], 1.5)
            st.progress(min(maturity_progress, 1.0))
            if abs(maturity_diff) <= 10:
                st.success("Vade hedefinde!")
            elif maturity_diff > 10:
                st.warning(f"Hedefin {maturity_diff:.0f}g uzeri")
            else:
                st.info(f"Hedefin {abs(maturity_diff):.0f}g alti")
    ...
    with g3:
        ...
        if approved:
            gross_progress = min(weighted_gross / s4["target_portfolio_gross_pct"], 1.5)
            st.progress(min(gross_progress, 1.0))
            if gross_diff >= 0:
                st.success(f"Brut kar hedefi asildi! (+{gross_diff:.2f}%)")
            elif gross_diff >= -2:
                st.warning(f"Hedefe yakin ({gross_diff:.2f}%)")
            else:
                st.error(f"Hedefin altinda ({gross_diff:.2f}%)")

    if approved:
        st.divider()
        st.subheader("Islem Bazli Detay")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Onaylanan Islem Sayisi", len(approved))
        m2.metric("Reddedilen Islem Sayisi", len([d for d in deals if d["Durum"] == "Reddedildi"]))
        ...
        st.info("Henuz onaylanan islem yok. Degerlendirme sekmesinden islem onaylayin.")

# ══════════════════════════════════════════════
# TAB 5: AYARLAR / WEIGHTS
# ══════════════════════════════════════════════
with tabs[4]:
    st.header("Ayarlar - BPS Agirliklari & NPL Parametreleri")
    ...
    with col2:
        ...
        w5["category_fmcg"] = st.number_input("FMCG (bps)", value=w5["category_fmcg"])
        w5["category_standard"] = st.number_input("Standart (bps)", value=w5["category_standard"])
        w5["category_textile"] = st.number_input("Tekstil (bps)", value=w5["category_textile"])
        ...
        st.subheader("NPL Prim Parametreleri")
        w5["npl_multiplier"] = st.number_input(
            "NPL Katsayisi",
            value=float(w5["npl_multiplier"]), step=0.1,
            help="(NPL% - Esik%) x Katsayi x 100 = BPS primi. Ornek: 0.5"
        )
        w5["npl_cap_bps"] = st.number_input(
            "NPL Prim Tavani (bps)",
            value=int(w5["npl_cap_bps"]), step=25,
            help="NPL priminin maksimum degeri. Ornek: 200 bps = +2%"
        )

        st.markdown("**NPL Prim Ornekleri (Mevcut Ayarlarla):**")
        npl_ex = []
        for npl_val in [1, 2, 3, 4, 5, 6, 7]:
            bps_ex = calculate_npl_premium_bps(npl_val, st.session_state.strategy["npl_threshold_pct"],
                                                w5["npl_multiplier"], w5["npl_cap_bps"])
            npl_ex.append({"NPL %": npl_val, "Prim (bps)": bps_ex, "Prim (%)": round(bps_ex/100, 2)})
        st.dataframe(pd.DataFrame(npl_ex), use_container_width=True, hide_index=True)
